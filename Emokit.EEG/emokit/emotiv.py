...
from .mevitae_output import EmotivMeVitaeOutput
...
class Emotiv(object):
    ...
    def __init__(self, display_output=False, serial_number=None, is_research=False, write=False,
                 ...
                 as_json = False, single_call = False, era_output = False):
        """
        ...
        # Display sensor values as json string
        self.as_json = as_json
        # Return the value only once 
        self.single_call = single_call
        # Send sensor values to Era.Server 
        self.era_output = era_output
    ...
    def initialize_era_output(self):
        print("Initializing Era Output Thread...")
        if self.era_output:
            self.output = EmotivMeVitaeOutput(serial_number=self.serial_number, old_model=self.old_model, as_json = self.as_json, single_call = self.single_call, era_output = self.era_output)

    def initialize_output(self):
        print("Initializing Output Thread...")
        if self.display_output:
            self.output = EmotivOutput(serial_number=self.serial_number, old_model=self.old_model, as_json = self.as_json, single_call = self.single_call)
            ...
            if not self.reader.data.empty():
                ...
                if not self.read_encrypted:
                    if not self.read_values:
                        ...
                        if self.display_output or self.era_output:
