...
class EmotivReader(object):
    ...
    def data_handler(self, data):
        ...
        if not self._stop_signal:
            ...
            _data = {}
            _data['systemmillisecond'] = current_milli_time()
            _data['data'] = data[1:]          
            self.save_data.put_nowait(_data)
            self.save_data_size += 1
            if data is not None:
                self.data.put_nowait(EmotivReaderTask(data=''.join(map(chr, _data['data'])), timestamp=_data['systemmillisecond']))
        else:
            self.lock.release()
